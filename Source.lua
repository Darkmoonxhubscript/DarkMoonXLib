...
local darkLib = {
  ...
  Themes = loadstring(game:HttpGet("https://raw.githubusercontent.com/Darkmoonxhubscript/DarkMoonXLib/main/Themes.lua"))(),
  Flags = {},
  Save = {
    Theme = "Default",
    UISize = {550, 310},
  ...
  Icons = loadstring(game:HttpGet("https://raw.githubusercontent.com/Darkmoonxhubscript/DarkMoonXLib/main/Icons.lua"))() --[[
...
local function GetIcon(GetName)
  ...
  for iconName,rbxassetid in pairs(darkLib.Icons) do
...
local function VerifyTheme(ThemeName)
  for name,__ in pairs(darkLib.Themes) do
...
local function LoadSettings(FileName)
  if readfile and isfile and isfile(FileName) then
    ...
    if decode and typeof(decode) == "table" then
      if FindTable(decode, "ScrollSize") then darkLib.Save["ScrollSize"] = decode["ScrollSize"]end
      if FindTable(decode, "UISize") then darkLib.Save["UISize"] = decode["UISize"]end
      if FindTable(decode, "Theme") and VerifyTheme(decode["Theme"]) then darkLib.Save["Theme"] = decode["Theme"]end
      if FindTable(decode, "TransparencyHub")then darkLib.Save["TransparencyHub"] = decode["TransparencyHub"]end
    end
  end
end;LoadSettings("dark library V4.lua")

-- Theme local
local Theme = darkLib.Themes[darkLib.Save.Theme]
...
local function insertTheme(instance, Theme)
  table.insert(darkLib.SaveForTheme[Theme], instance)
...
local function Corner(props, Circle)
  ...
  if not Circle then table.insert(darkLib.SaveForTheme.Corner, New)end
  return New
end
local function Stroke(props)
  ...
  table.insert(darkLib.SaveForTheme.Stroke, New)
  return New
end
local function Button(parent, props, childrens)
  ...
  table.insert(darkLib.SaveForTheme.Buttons, New)
  return New
end
...
task.spawn(function()
  ...
  function darkLib:MakeNotify(Configs)
...
-- Themes
function darkLib:GetIcon(IconName)
  return GetIcon(IconName)
end
function darkLib:SetTransparency(NewVal)
  local NewVal = math.clamp(NewVal, 0, 0.5)
  darkLib.Save.TransparencyHub = NewVal
  
  local Save = darkLib.SaveForTheme
...
function darkLib:CreateTheme(ThemeName, Configs)
  darkLib.Themes[ThemeName] = Configs
end
function darkLib:SetTheme(themeName)
  if not VerifyTheme(themeName) then return end
  
  Theme, darkLib.Save.Theme = darkLib.Themes[themeName], themeName
  Theme["Transparency"] = darkLib.Save.TransparencyHub
  
  local Save = darkLib.SaveForTheme
  SaveSenttigs("dark library V4.lua", darkLib.Save)
...
function darkLib:GetThemes()
  local Themes = {}
  table.foreach(darkLib.Themes, function(ThemeName)
...
-- Window
function darkLib:Visible(Bool)ScreenGui.Enabled = Bool end
function darkLib:Destroy()SreenGui:Destroy()end
function darkLib:MakeWindow(Configs)
  local Minimized, WaitMinimize, SavedSize
  local WTitle = Configs[1] or Configs.Title or "dark library"
  local SubTitle = Configs[2] or Configs.SubTitle or "by : dark9999"
  local LoadTitle = Configs[3] or Configs.LoadText or SubTitle
  local DefaultTheme = Configs[4] or Configs.DefaultTheme or false
  local SaveCfg = Configs[5] or Configs.Flags or "dark Library v4 | Flags.lua"
  local Flags = darkLib.Flags
  ...
  local MainFrame = MakeDrag(insertTheme(Create("Frame", ScreenGui, {
    Size = UDim2.fromOffset(unpack(darkLib.Save.UISize)),
    Position = UDim2.new(0.5, -darkLib.Save.UISize[1]/2, 0.5, -darkLib.Save.UISize[2]/2),
    Active = true,
    BackgroundColor3 = Theme["Color Hub 1"],
    BackgroundTransparency = Theme["Transparency"]
  }, {Corner()}), "Frames"))
  ...
  local MainScroll = Create("ScrollingFrame", MainFrame, {
    Size = UDim2.new(0, darkLib.Save.ScrollSize, 1, -Topbar.Size.Y.Offset),
  ...
  ConnectSave(ControlSize1, function()
    if not Minimized then
      darkLib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
      SaveSenttigs("dark library V4.lua", darkLib.Save)
    end
  end)
  ConnectSave(ControlSize2, function()
    darkLib.Save.ScrollSize = MainScroll.Size.X.Offset
    SaveSenttigs("dark library V4.lua", darkLib.Save)
  end)
  ...
  function Window:Set(NewVal)
    ...
    elseif typeof(NewVal) == "number" then
      darkLib:SetTransparency(NewVal)
    end
  end
